...
import { Briefcase, ChevronRight, Plus, Edit, Trash2, Search, ChevronLeft, Server } from 'lucide-react';
...
interface ServiceManagementProps {
  onSelectService: (moduleName: string) => void;
  user: User;
}
...
const ServiceManagement: React.FC<ServiceManagementProps> = ({ onSelectService, user }) => {
  const [services, setServices] = useState<Module[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingService, setEditingService] = useState<Module | null>(null);
  ...
  const fetchServices = useCallback(async () => {
    ...
    try {
        ...
        const response = await apiClient<{ data: { services: Module[], total: number, page: number, totalPages: number } }>(`/modules?${params.toString()}`);
  ...
  const handleDelete = async (e: React.MouseEvent, service: Module) => {
    e.stopPropagation();
...
const ServiceFormModal: React.FC<{
    isOpen: boolean; onClose: () => void; onSave: (data: {name: string, description: string}) => void; service: Module | null;
